...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    console.warn("[BOL] Auth check in improvement:", err);
  }
  ...
  // Step 2 — fetch latest confidence score if user is logged in
  let score = 0;
  if (currentUser) {
    score = await fetchScoreFromSupabase(currentUser.id);
  }
  ...
  // Wire streak card for guest prompt
  const streakCard = document.querySelector('.streak-card');
  if (streakCard) {
    streakCard.style.cursor = 'pointer';
    streakCard.addEventListener('click', () => {
      if (!currentUser && window.BOLAuth) {
        window.BOLAuth.openSaveProgressPrompt();
      }
    });
  }
});
...
async function loadSession(id, score) {
  try {
    ...
    const resolvedScore = score > 0 ? score : (Number(data.session?.score || data.session?.confidence_score) || 0);
    populateImprovement(data.session, resolvedScore);
...
async function submitDrillAttempt(drillId, drillConfig, overlay) {
  ...
  if (checkConsecutivePasses(drillId)) {
    ...
    // 🔥 SAVE TO DATABASE
    if (!currentUser) {
      console.log("Guest completed drill — progress saved in local session only");
    } else {
